                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-sm text-gray-500">
                            {rule.action_type === "create_task"
                              ? `${formatDeliveryTiming(rule.delivery_offset_days)} · ${formatDueTiming(rule.due_offset_days)} · Manager warning ${rule.overdue_grace_days ? `${rule.overdue_grace_days} day${rule.overdue_grace_days === 1 ? "" : "s"} after due` : "when overdue"} · ${formatAssignment(rule)}${rule.cancel_on_pipeline_advance ? " · Cancels when pipeline advances" : ""}`
...
function formatDueTiming(days: number) {
  if (days === 0) return "Due when delivered";
  return `Due ${days} day${days === 1 ? "" : "s"} after delivery`;
}

function formatDeliveryTiming(days: number) {
