  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await onSave({ name, trigger_event: triggerEvent, trigger_value: valueOptions.length ? triggerValue : null,
        ...
        task_title: actionType === "create_task" ? taskTitle : null, task_priority: taskPriority, task_type_id: taskTypeId || null, due_offset_days: dueOffsetDays, overdue_grace_days: overdueGraceDays,
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-xl"><form onSubmit={submit}>
    ...
    <div className="grid gap-5 py-6">
      ...
      {actionType === "create_task" ? <>
        ...
        <Field label="Task due after"><div className="flex items-center gap-3"><Input type="number" min={0} max={365} value={dueOffsetDays} onChange={(e) => setDueOffsetDays(Math.min(365, Math.max(0, Number(e.target.value) || 0)))} className="max-w-28"/><span className="text-sm text-gray-600">{dueOffsetDays ? `${dueOffsetDays} day(s)` : "Today"}</span></div><span className="text-xs font-normal text-gray-500">The task appears immediately. This sets the due date shown to the employee.</span></Field>
