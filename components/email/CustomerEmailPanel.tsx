...
type Props = { jobId: string; recipient: string; emails: CustomerEmail[]; templates: EmailTemplate[];
  recipientOptions?: { label: string; email: string }[]; attachments: JobAttachment[]; canSend: boolean; compact?: boolean; showHistory?: boolean };

export default function CustomerEmailPanel({ jobId, recipient, recipientOptions = [], emails, templates, attachments, canSend, compact = false, showHistory = true }: Props) {
  ...
  return <div className={`grid ${showHistory ? "xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.8fr)]" : ""} ${compact ? "gap-3" : "gap-5"}`}>
    <div className={`rounded-lg border border-blue-200 bg-white shadow-sm ${compact ? "p-4" : "p-5"}`}>
      ...
      {canSend ? <div className="mt-5 grid gap-4">
        ...
        <label className="grid gap-1.5 text-sm font-medium">Message<textarea value={body} onChange={(e) => setBody(e.target.value)} rows={5} className="rounded-lg border border-gray-300 p-3 text-sm" placeholder="Write the customer email..."/></label>
        {availableAttachments.length ? <details><summary className="flex cursor-pointer items-center gap-2 text-sm font-medium"><Paperclip className="h-4 w-4"/>Attach existing job files</summary><div className="mt-2 max-h-36 space-y-2 overflow-y-auto rounded-lg border p-3">{availableAttachments.map((item) => <label key={item.id} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={attachmentIds.includes(item.id)} onChange={(e) => setAttachmentIds((current) => e.target.checked ? [...current, item.id] : current.filter((id) => id !== item.id))}/><span className="truncate">{item.file_name}</span></label>)}</div></details> : null}
    ...
    {showHistory ? <div className={`rounded-lg border border-gray-200 bg-white shadow-sm ${compact ? "p-4" : "p-5"}`}>
