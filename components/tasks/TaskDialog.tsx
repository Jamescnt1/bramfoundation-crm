...
import { TASK_PRIORITIES, TASK_STATUSES, type TaskNote, type TaskPriority, type TaskStatus, type TaskType, type UniversalTask } from "./types";
...
type Props = {
  ...
  currentEmployeeId?: string | null; currentEmployeeRole?: string | null;
...
function TaskDialogForm({ onOpenChange, onSaved, task, customers, jobs, employees, taskTypes, defaultCustomerId, defaultJobId, currentEmployeeId = null, currentEmployeeRole = null, onLatestNoteChange }: Omit<Props, "open">) {
  ...
  const handleLatestNoteChange = useCallback((note: TaskNote | null) => {
    if (taskId) onLatestNoteChange?.(taskId, note);
  }, [onLatestNoteChange, taskId]);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      onSaved({ ...saved, latest_note: task?.latest_note ?? null });
      onOpenChange(false);
    }
  ...
  return <DialogContent className="max-h-[calc(100dvh-1rem)] overflow-y-auto overscroll-contain sm:max-w-2xl">
