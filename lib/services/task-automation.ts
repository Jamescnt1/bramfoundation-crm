...
export type AutomationRule = {
  ...
  trigger_status: PipelineStage | null; task_title: string | null; delivery_offset_days: number; due_offset_days: number;
...
export type AutomationRuleValues = {
  ...
  action_type: AutomationActionType; target_status: PipelineStage | null;
  task_title: string | null; delivery_offset_days: number; due_offset_days: number; overdue_grace_days: number; assignment_type: AutomationAssignmentType;
  task_priority: "low" | "normal" | "high" | "urgent";
  task_type_id: string | null;
...
function normalize(values: AutomationRuleValues) {
  return {
    ...
    delivery_offset_days: values.action_type === "create_task" ? Math.max(0, Math.trunc(values.delivery_offset_days)) : 0,
