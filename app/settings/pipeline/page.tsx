...
import ProductionWorkflowSettings from "@/components/settings/ProductionWorkflowSettings";

export const dynamic = "force-dynamic";

export default async function PipelineSettingsPage() {
  await requirePermission("pipeline.settings.manage");
  const [stages, settingsResult] = await Promise.all([
    getPipelineStages({ includeArchived: true }),
    createAdminClient().from("production_workflow_settings").select("enabled").eq("singleton", true).maybeSingle(),
  ]);
  ...
  return (
    <main className="min-h-screen bg-gray-50 p-6 md:p-8">
      <div className="mx-auto max-w-5xl">
        ...
        />
        <ProductionWorkflowSettings enabled={settingsResult.data?.enabled ?? true} />
